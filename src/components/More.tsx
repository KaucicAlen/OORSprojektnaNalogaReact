...
function More(props: projektiProps) {
            ...
            <div className="btn">
                <ul>
                    <li><button className="btn1" onClick={() => {
                        let el = { ...projekti };
                        let novaVrednost = 1;
                        let razlog = prompt("Razlog za prekinitev?");
                        let staraVrednost = el.stanjeGradnje;

                        if (novaVrednost == null || razlog == null) {
                            alert("Napaka pri vnosu")
                            el.stanjeGradnje = staraVrednost;
                            setProjekti(el);
                            let novoPolje = props.projekti;
                            for (let i = 0; i < novoPolje.length; i++) {
                                if (novoPolje[i].id == id) {
                            ...
                            el.stanjeGradnje = novaVrednost;
                            el.razlogZaPrekinitev = razlog
                            setProjekti(el);
                            let novoPolje = props.projekti;
                            for (let i = 0; i < novoPolje.length; i++) {
                                if (novoPolje[i].id == id) {
                                    novoPolje[i] = el;
                                    props.onChange(novoPolje)
                                    alert("Projkt upešno prekinjen")
                                }
                            }
                        }

                    }}>Prekini Gradnjo</button></li>
                        ...
                        let novaVrednost = 2;
                            ...
                            for (let i = 0; i < novoPolje.length; i++) {
                                if (novoPolje[i].id == id) {
                                    novoPolje[i] = el;
                                    props.onChange(novoPolje)
                                    alert("Projekt upešno zaključen")
                                }
                            }
                    }}>Zaključi Gradnjo</button></li>
